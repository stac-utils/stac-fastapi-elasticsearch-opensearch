...
async def _format_catalogs_with_links(
    ...
    if unique_parent_ids:
        ...
        async def fetch_title(pid):
            try:
                parent_catalog = await client.database.find_catalog(pid)
    ...
    async def fetch_children(catalog_id):
        try:
            children_data, _, _ = await client.database.get_catalog_children(
            ...
            logger.warning(f"Could not fetch children for catalog {catalog_id}: {e}")
            return catalog_id, []
    ...
    for catalog_data in catalogs_data:
        ...
        catalog_stac = client.catalog_serializer.db_to_stac(
            catalog_data,
            request,
            extensions=_active_extensions(client),
        )
        ...
        for link in catalog_dict.get("links", []):
            if link.get("rel") in ["parent", "related"] and "title" not in link:
                ...
                if link.get("href") == base_url or link.get("href") == f"{base_url}/":
                ...
                for pid in catalog_data.get("parent_ids", []):
                    if pid in parent_id_to_title and f"/catalogs/{pid}" in link.get(
                        "href", ""
                    ):
...
async def catalogs(
    ...
    (catalogs_list, next_token, total_hits,) = await client.database.get_all_catalogs(
        token=token,
        limit=limit,
        request=request,
    ...
    catalog_stac_objects = await _format_catalogs_with_links(
        client, catalogs_list, request, base_url
    )
    ...
    if next_token:
        links.append(
            {
                "rel": "next",
                "href": f"{base_url}/catalogs?{urlencode({'limit': limit, 'token': next_token})}",
                "type": "application/json",
                "title": "Next page",
            }
        )
...
async def create_catalog(
    client: BaseCoreClient, catalog: Catalog, request: Request
) -> Catalog:
    ...
    db_catalog = client.catalog_serializer.stac_to_db(catalog, request)
    ...
    await client.database.create_catalog(db_catalog_dict, refresh=True)

    # Return the created catalog with extensions for proper serialization
    return client.catalog_serializer.db_to_stac(
        db_catalog_dict,
        request,
        extensions=[type(ext).__name__ for ext in client.database.extensions],
    )


async def update_catalog(
    client: BaseCoreClient, catalog_id: str, catalog: Catalog, request: Request
) -> Catalog:
    ...
    try:
        ...
        existing_catalog_db = await client.database.find_catalog(catalog_id)
        ...
        updated_db_catalog = await client.database.find_catalog(catalog_id)
        return client.catalog_serializer.db_to_stac(
            updated_db_catalog,
            request,
            extensions=_active_extensions(client),
...
async def get_catalog(
    ...
    try:
        # 1. FETCH DATA
        db_catalog = await client.database.find_catalog(catalog_id)

        # 2. SERIALIZE & INITIALIZE DICT
        catalog = client.catalog_serializer.db_to_stac(
            db_catalog, request, extensions=_active_extensions(client)
        )
        base_url = str(request.base_url).rstrip("/")
        ...
        root_id = settings.get("STAC_FASTAPI_LANDING_PAGE_ID", "stac-fastapi")
        ...
        try:
            ...
            while True:
                (
                    ...
                    _,
                    next_token,
                ) = await client.database.get_catalog_children(
                    catalog_id=catalog_id,
                    limit=limit,
                    token=current_token,
                    request=request,
...
async def delete_catalog(
    client: BaseCoreClient, catalog_id: str, request: Request, settings: dict
) -> None:
    ...
    try:
        # Verify the catalog exists
        await client.database.find_catalog(catalog_id)

        # Find all collections with this catalog in parent_ids
        # Get all collections (no pagination limit) by using a large limit
        children, _, _ = await client.database.get_catalog_collections(
            catalog_id=catalog_id,
            limit=10000,
            token=None,
            request=request,
        ...
        for child in children:
            ...
            try:
                ...
                if catalog_id in parent_ids:
                    ...
                    await client.database.update_collection(
        ...
        await client.database.delete_catalog(catalog_id, refresh=True)
...
async def get_catalog_collections(
    ...
    try:
        ...
        # 2. Query collections by parent_ids field with pagination
        (
            collections_data,
            total_hits,
            next_token,
        ) = await client.database.get_catalog_collections(
            catalog_id=catalog_id, limit=limit, token=token, request=request
        )

        # 3. Serialize the results (Point 1: Optimized N+1 Fix)
        ...
        for collection_db in collections_data:
            try:
                ...
                collection = client.collection_serializer.db_to_stac_in_catalog(
                    collection_db,
                    request,
                    catalog_id=catalog_id,
                    extensions=_active_extensions(client),
        ...
        # 5. Generate next link if more results exist
        if next_token:
...
async def get_catalog_catalogs(
    ...
    try:
        ...
        (
            ...
            total_hits,
            next_token,
        ) = await client.database.get_catalog_catalogs(
            catalog_id=catalog_id, limit=limit, token=token, request=request
        )

        base_url = str(request.base_url).rstrip("/")
        catalogs_list = await _format_catalogs_with_links(
            client, catalogs_data, request, base_url
        )
        ...
        if next_token:
            links.append(
                {
                    "rel": "next",
                    "href": f"{base_url}/catalogs/{catalog_id}/catalogs?{urlencode({'limit': limit, 'token': next_token})}",
                    "type": "application/json",
                }
            )
...
async def create_catalog_catalog(
    ...
    try:
        ...
        try:
            existing_catalog = await client.database.find_catalog(catalog.id)
            ...
            if catalog_id not in parent_ids_set:
                ...
                # Persist the update
                await client.database.create_catalog(existing_catalog, refresh=True)
                logger.info(
...
async def create_catalog_collection(
    ...
    try:
        ...
        # Get collection ID safely (handle both dict and Pydantic model)
        c_id = collection["id"] if isinstance(collection, dict) else collection.id

        # Check if the collection already exists in the database
        try:
            existing_collection_db = await client.database.find_collection(c_id)
            ...
            updated_collection = client.collection_serializer.db_to_stac_in_catalog(
            ...
            # We assume if it's a dict or stac_pydantic.Collection, it's a full collection object to be created.
            # If it's CollectionId (or other), it's just a reference.
            is_full_collection = isinstance(collection, (dict, Collection))

            if not is_full_collection:
                raise HTTPException(
                    status_code=404, detail=f"Collection {c_id} not found."
                )
            ...
            collection_db = client.collection_serializer.stac_to_db(
                collection_dict, request
            )
            # Convert to dict if it's a Pydantic model
            if hasattr(collection_db, "model_dump"):
                collection_db_dict = collection_db.model_dump(mode="json")
            else:
                collection_db_dict = collection_db

            await client.database.create_collection(
                collection=collection_db_dict, refresh=True
            )

            # Convert back to STAC format for the response
            created_collection = client.collection_serializer.db_to_stac_in_catalog(
                collection_db_dict,
                request,
                catalog_id=catalog_id,
                extensions=_active_extensions(client),
...
async def get_catalog_collection(
    ...
    try:
        await client.database.find_catalog(catalog_id)
        collection_db = await client.database.find_collection(collection_id)
        ...
        return client.collection_serializer.db_to_stac_in_catalog(
            collection_db,
            request,
            catalog_id=catalog_id,
            extensions=_active_extensions(client),
...
async def get_catalog_collection_items(
    ...
    try:
        await client.database.find_catalog(catalog_id)
        return await client.item_collection(
...
async def get_catalog_children(
    ...
    try:
        ...
        if catalog_docs:
            formatted_catalogs_list = await _format_catalogs_with_links(
                client, catalog_docs, request, base_url
            )
            ...
            if doc.get("type") == "Catalog":
                ...
                if formatted_catalog is not None:
                    ...
                    formatted_children.append(
                        client.catalog_serializer.db_to_stac(
                            doc,
                            request,
                            extensions=_active_extensions(client),
                        )
                ...
                formatted_children.append(
                    client.collection_serializer.db_to_stac(doc, request)
                )
        ...
        if next_token:
            params = {"limit": limit, "token": next_token}
            if resource_type:
...
class CatalogsExtension(ApiExtension):
    ...
    async def catalogs(
        ...
        return await catalogs(
            client=self.client, request=request, limit=limit, token=token
        )

    async def create_catalog(self, catalog: Catalog, request: Request) -> Catalog:
        """Create a new catalog."""
        return await create_catalog(
            client=self.client, catalog=catalog, request=request
        )
    ...
    async def get_catalog_collections(
        ...
        """Get collections linked from a specific catalog with pagination."""
        return await get_catalog_collections(
            client=self.client,
            catalog_id=catalog_id,
            request=request,
            limit=limit,
    ...
    async def get_catalog_catalogs(
        ...
        """Get all sub-catalogs of a specific catalog with pagination."""
        return await get_catalog_catalogs(
            client=self.client,
            catalog_id=catalog_id,
            request=request,
            limit=limit,
    ...
    async def create_catalog_catalog(
        ...
        """Create a new catalog or link an existing catalog as a sub-catalog."""
        return await create_catalog_catalog(
            client=self.client,
            catalog_id=catalog_id,
            catalog=catalog,
            request=request,
    ...
    async def create_catalog_collection(
        self,
        catalog_id: str,
        collection: CollectionId | stac_types.Collection,
        request: Request,
    ) -> stac_types.Collection:
        """Create a new collection and link it to a specific catalog."""
        return await create_catalog_collection(
            client=self.client,
            catalog_id=catalog_id,
    ...
    async def get_catalog_collection(
        ...
        """Get a specific collection from a catalog."""
        return await get_catalog_collection(
            client=self.client,
            catalog_id=catalog_id,
            collection_id=collection_id,
            request=request,
    ...
    async def get_catalog_collection_items(
        ...
        """Get items from a collection in a catalog."""
        return await get_catalog_collection_items(
            client=self.client,
            catalog_id=catalog_id,
            collection_id=collection_id,
            request=request,
    ...
    async def get_catalog_collection_item(
        ...
        """Get a specific item from a collection in a catalog."""
        return await get_catalog_collection_item(
            client=self.client,
            catalog_id=catalog_id,
            collection_id=collection_id,
            item_id=item_id,
    ...
    async def get_catalog_children(
        ...
        """Get all children (Catalogs and Collections) of a specific catalog."""
        return await get_catalog_children(
            client=self.client,
            catalog_id=catalog_id,
            request=request,
            limit=limit,
    ...
    async def delete_catalog_collection(
        ...
        """Delete a collection from a catalog (Unlink only)."""
        return await delete_catalog_collection(
            client=self.client,
            catalog_id=catalog_id,
            collection_id=collection_id,
            request=request,
