...
async def resolve_cql2_indexes(
    ...
    for collection_item, date_range in cql2_metadata:
        ...
        logger.debug(f"Processing collections: {collections} with date_range: {date_range}")
        ...
        if date_range:
            ...
            for collection in collections:
                ...
                # If any range returns no indexes, we need to use wildcard
                if not index_list:
                    logger.info(f"Range {date_range} returned no indexes for collection {collection}, using wildcard")
                    print(f"Range {date_range} returned no indexes for collection {collection}, using wildcard")
                    use_wildcard = True
            ...
            # If there's no date range for ANY collection, we need to search all indexes
    ...
    # If any range requires wildcard search, use the default wildcard pattern
    if use_wildcard:
        logger.info(f"At least one range requires wildcard search, using default: {ITEM_INDICES}")
        print(f"At least one range requires wildcard search, using default: {ITEM_INDICES}")
...
def build_cql2_filter(queryables_mapping: Dict, filter: Dict) -> Tuple[Dict, List]:
    ...
    print(f"Filter to be processed: {json.dumps(filter, indent=2) if isinstance(filter, dict) else filter}")
    ...
    print(f"es_query to be processed: {json.dumps(es_query, indent=2) if isinstance(es_query, dict) else es_query}")
