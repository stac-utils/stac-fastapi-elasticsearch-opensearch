...
| `FREE_TEXT_FIELDS` | Comma-separated list of fields to search in free-text queries (e.g., `properties.title,properties.standard_name,properties.description`). Supports field boosting syntax (e.g., `properties.title^3` gives title 3x weight). If not set, defaults to: `id,collection,properties.title^3,properties.description,properties.keywords`. | Default fields with title boosting | Optional |
