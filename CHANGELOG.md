...
- `PUT /catalogs/{catalog_id}/collections/{collection_id}` now delegates to the core collection update after its catalog-membership check: it returns `400` when the body `id` differs from the URI and applies the same validation and serialization as `PUT /collections/{collection_id}`. [#813](https://github.com/stac-utils/stac-fastapi-elasticsearch-opensearch/issues/813)
- With `ENABLE_TRANSACTIONS_EXTENSIONS=false`, the catalog write routes (`POST /catalogs`, `PUT`/`DELETE /catalogs/{catalog_id}`, `POST /catalogs/{catalog_id}/collections`, `PUT`/`DELETE /catalogs/{catalog_id}/collections/{collection_id}`, `POST /catalogs/{catalog_id}/catalogs` and `DELETE /catalogs/{catalog_id}/catalogs/{sub_catalog_id}`) and the `multi-tenant-catalogs/transaction` conformance class are no longer exposed when `ENABLE_CATALOGS_ROUTE=true`. Defaults are unchanged. [#813](https://github.com/stac-utils/stac-fastapi-elasticsearch-opensearch/issues/813)
...
- `POST /collections/{collection_id}/bulk_items` now reports values in `items` that are not JSON objects, or that lack a non-empty string `id`, per item in `errors` (keyed by their `items` key) and writes the rest of the batch; with `RAISE_ON_BULK_ERROR=true` the batch is rejected with `400` and nothing is written. Previously these entries returned `500` or were silently counted as skipped. Valid bulk inserts are unchanged. [#865](https://github.com/stac-utils/stac-fastapi-elasticsearch-opensearch/issues/865)
- `POST /collections-search` now returns `400` instead of `500` when body fields have invalid types or values, such as a non-numeric `limit`, malformed `bbox` or invalid `datetime`. Valid collection searches are unchanged. [#865](https://github.com/stac-utils/stac-fastapi-elasticsearch-opensearch/issues/865)
...
- `PUT /collections/{collection_id}` and validator-enabled `PATCH /collections/{collection_id}` no longer drop a collection's catalog memberships in Elasticsearch and OpenSearch. Stored `parent_ids` are kept, a `parent_ids` in the request body is ignored, and the write is conditional on the version read, retrying on a concurrent link or unlink and returning `409` if conflicts persist. [#813](https://github.com/stac-utils/stac-fastapi-elasticsearch-opensearch/issues/813)
- `POST /collections`, `PUT /collections/{collection_id}` and `PATCH /collections/{collection_id}` responses now include the catalog `related`/`parent` links and other extension links that `GET /collections/{collection_id}` returns; the extension names were being passed as `"str"`. [#813](https://github.com/stac-utils/stac-fastapi-elasticsearch-opensearch/issues/813)
...
- Catalog writes no longer overwrite concurrent writes in Elasticsearch and OpenSearch. Linking and unlinking a sub-catalog (`POST /catalogs/{catalog_id}/catalogs` with `{"id"}`, `DELETE /catalogs/{catalog_id}/catalogs/{sub_catalog_id}`) update only `parent_ids` in one atomic scripted update. `PUT /catalogs/{catalog_id}` keeps the stored `parent_ids` and writes conditionally on the version read, retrying on a concurrent link or unlink. Both return `409` if conflicts persist. `create_catalog` is now create-only. [#813](https://github.com/stac-utils/stac-fastapi-elasticsearch-opensearch/issues/813)
