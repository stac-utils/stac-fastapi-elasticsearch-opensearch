...
- Updated `stac-fastapi-catalogs-extension` to `v1.0.0`. Conformance classes now advertise `v1.0.0` URIs (replacing `v1.0.0-rc.*`), and every entity in the `/catalogs/{catalog_id}/children` response now includes `root`, `self`, and `parent` links per STAC API - Children v1.0.0.
