...
- Item search, catalog search and collection item listing now return `400` for malformed pagination tokens, meaning any token that is not a base64-encoded JSON list with one value per sort field. Tokens that decode to `null`, and on OpenSearch to `{}` or `[]`, previously restarted at the first page and are now rejected. Valid next tokens and collection/catalog cursors are unchanged. [#865](https://github.com/stac-utils/stac-fastapi-elasticsearch-opensearch/issues/865)
