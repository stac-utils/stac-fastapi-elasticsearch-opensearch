...
{{/*
Name for the autogenerated OpenSearch admin secret.
*/}}
{{- define "stac-fastapi.opensearchAdminSecretName" -}}
{{- if .Values.opensearchSecurity.secretName }}
{{- .Values.opensearchSecurity.secretName }}
{{- else }}
{{- printf "%s-opensearch-admin" (include "stac-fastapi.fullname" .) }}
{{- end }}
{{- end }}

{{/*
Mutate values to ensure OpenSearch initial admin credentials are wired when
password generation is enabled.
*/}}
{{- define "stac-fastapi.configureOpensearchSecurity" -}}
{{- if and (eq .Values.backend "opensearch") (.Values.opensearch.enabled) (.Values.opensearchSecurity.generateAdminPassword | default false) }}
  ...
  {{- $flags := dict "hasPassword" false "hasUsername" false -}}
  {{- range $env := (default (list) .Values.opensearch.extraEnvs) }}
    {{- if eq (get $env "name") "OPENSEARCH_INITIAL_ADMIN_PASSWORD" }}
      {{- $_ := set $flags "hasPassword" true -}}
    {{- end }}
    {{- if eq (get $env "name") "OPENSEARCH_INITIAL_ADMIN_USERNAME" }}
      {{- $_ := set $flags "hasUsername" true -}}
    {{- end }}
  {{- end }}
  {{- if not (get $flags "hasPassword") }}
    {{- $passwordEnv := dict "name" "OPENSEARCH_INITIAL_ADMIN_PASSWORD" "valueFrom" (dict "secretKeyRef" (dict "name" $secretName "key" $passwordKey)) -}}
    {{- $_ := set .Values.opensearch "extraEnvs" (append (default (list) .Values.opensearch.extraEnvs) $passwordEnv) -}}
  {{- end }}
  {{- if not (get $flags "hasUsername") }}
    {{- $usernameEnv := dict "name" "OPENSEARCH_INITIAL_ADMIN_USERNAME" "valueFrom" (dict "secretKeyRef" (dict "name" $secretName "key" $usernameKey)) -}}
    {{- $_ := set .Values.opensearch "extraEnvs" (append (default (list) .Values.opensearch.extraEnvs) $usernameEnv) -}}
  {{- end }}
{{- end }}
{{- end }}
...
{{- if and (eq .Values.backend "opensearch") (get $opensearchSecurity "generateAdminPassword") }}
  ...
  {{- if or (not (hasKey $dbAuth "existingSecret")) (not $dbAuth.existingSecret) }}
    {{- $_ := set $dbAuth "existingSecret" $secretName -}}
  {{- end }}
  {{- if not (hasKey $dbAuth "usernameKey") }}
    {{- $_ := set $dbAuth "usernameKey" (default "username" (get $opensearchSecurity "usernameKey")) -}}
  {{- end }}
  {{- if not (hasKey $dbAuth "passwordKey") }}
    {{- $_ := set $dbAuth "passwordKey" (default "password" (get $opensearchSecurity "passwordKey")) -}}
  {{- end }}
{{- end }}
- name: BACKEND
